...
use mgpu::{
    AsyncCompute, AttachmentStoreOp, Binding, BindingSet, BindingSetDescription, BindingSetElement,
    BindingSetLayout, BindingSetLayoutInfo, BindingType, Buffer, BufferDescription,
    BufferUsageFlags, CompareOp, ComputePassDescription, ComputePipeline,
...
fn main() {
    ...
    struct CubeApplication {
        ...
        compute_pass_data: Buffer,
        cube_index_buffer: Buffer,
        pipeline: GraphicsPipeline,
        compute_pipeline: ComputePipeline,
        compute_binding_set: BindingSet,
    ...
    impl ApplicationHandler for CubeApplication {
        fn resumed(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
            let window = event_loop
                .create_window(WindowAttributes::default().with_title("Compute Example"))
                .unwrap();
            self.swapchain = Some(
                self.device
        ...
        fn window_event(
            ...
            match event {
                ...
                WindowEvent::RedrawRequested => {
                    ...
                    self.device
                        .write_buffer(
                            self.compute_pass_data,
                            &self
                                .compute_pass_data
                                .write_all_params(bytemuck::cast_slice(&[0.0f32])),
                        )
                        .unwrap();
                        ...
                        .unwrap();
                    {
                        let mut compute_command_recorder =
                            self.device.create_command_recorder::<AsyncCompute>();
                        {
