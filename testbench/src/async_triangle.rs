...
use gpu::{
    BeginComputePassInfo, Binding2, BufferCreateInfo, BufferHandle, BufferUsageFlags,
    ColorAttachment, CommandBuffer, ComputePassFlags, CullMode, ImageAspectFlags, ImageCreateInfo,
    ImageFormat, ImageHandle, ImageUsageFlags, ImageViewCreateInfo, ImageViewHandle, IndexType,
...
const COMPUTE_SHADER: &[u32] = glsl!(
    source = "
#version 460

layout(local_size_x = 32, local_size_y = 32, local_size_z = 1) in;
layout(set = 0, binding = 0, rgba8) uniform image2D multiply;

void main() {
    ivec2 loc = ivec2(gl_LocalInvocationID.x, gl_LocalInvocationID.y);
    imageStore(multiply, loc, vec4(1.0, 0.0, 0.0, 1.0));
}
    ",
    kind = compute,
    entry_point = "main"
...
layout(set = 0, binding = 1) uniform sampler2D multiply;
...
pub struct TriangleApp {
    ...
    mul_texture: ImageHandle,
    mul_texture_view: ImageViewHandle,

    david_sampler: SamplerHandle,

    compute_module: ShaderModuleHandle,
...
impl App for TriangleApp {
        ...
        Self: Sized,
    {
        ...
        let compute_module = app_state
            .gpu
            .make_shader_module(&gpu::ShaderModuleCreateInfo {
                label: Some("Triangle compute shader"),
                code: bytemuck::cast_slice(COMPUTE_SHADER),
            })?;
        ...
        let mul_texture = app_state.gpu.make_image(
            &ImageCreateInfo {
                label: Some("Mul image"),
                width: 32,
                height: 32,
                depth: 1,
                mips: 1,
        ...
        let mul_texture_view = app_state.gpu.make_image_view(&ImageViewCreateInfo {
            label: Some("Mul image view"),
            image: mul_texture,
            view_type: gpu::ImageViewType::Type2D,
            format: ImageFormat::Rgba8,
            components: gpu::ComponentMapping::default(),
    ...
    fn draw<'a>(
        ...
        {
            let mut compute = gpu.start_command_buffer(gpu::QueueType::AsyncCompute)?;
